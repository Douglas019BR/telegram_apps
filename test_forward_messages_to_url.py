...
def mocked_telegram_client_connect(mocker):
    return mocker.patch("forward_messages_to_url.TelegramClient.connect",new_callable=AsyncMock)

@pytest.fixture()
def mocked_telegram_client_start(mocker):
    return mocker.patch("forward_messages_to_url.TelegramClient.start",new_callable=AsyncMock)


@pytest.fixture()
def mocked_telegram_client_run_until_disconnected(mocker):
    return mocker.patch("forward_messages_to_url.TelegramClient.run_until_disconnected", return_value=None, new_callable=AsyncMock)


@pytest.mark.asyncio
async def test_handle_new_message_success(mocked_group_id, mocked_requests, mock_event, mock_response_200):
...
@pytest.mark.asyncio
async def test_handle_new_message_error(mocked_group_id, mocked_requests, mock_event, mock_response_error):
...
@pytest.mark.asyncio
async def test_telegram_connection(mocked_telegram_client_connect,
